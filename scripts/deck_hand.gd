...
func get_tutorial_text_for_phase(phase: String) -> String:
	if(phase == "USUAL_DRAW"):
		return "Presiona el botón de ROBAR CARTA para obtener un hechizo"
	if(phase == "USUAL_PLAY"):
		return "Haz click sobre la carta de FUEGO para usarla como ataque"
	if(phase == "QUANTIC_DRAW"):
		return "Presion el botón de ROBAR CARTA para obtener una carta cuántica"
	if(phase == "QUANTIC_PLAY"):
		return "Inserta la carta X o H en el centro para que los qubits de entrada sean iguales a los de la carta FUEGO; en este caso 111"
	if(phase == "QUANTIC_RESOLVE"):
		return "Presiona el botón de RESOLVER para obtener los qubits resultado"
	if(phase == "QUANTIC_MEASURE"):
		return "Presiona el botón MEDIR para obtener el valor de los qubits en superposición"
	return ""
