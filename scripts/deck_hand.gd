...
func get_tutorial_text_for_phase(phase: String) -> String:
	if(phase == "USUAL_DRAW"):
		return "Presiona el boton de ROBAR CARTA para obtener un hechizo"
	if(phase == "USUAL_PLAY"):
		return "Haz click sobre la carta de FUEGO para usarla como ataque"
	if(phase == "QUANTIC_DRAW"):
		return "Presion el boton de ROBAR CARTA para obtener una carta cuántica"
	if(phase == "QUANTIC_PLAY"):
		return "Inserta la carta X o H en el centro para que los qubits de entrada sean iguales a los de la carta FUEGO; en este caso 1 1 1"
	if(phase == "QUANTIC_RESOLVE"):
		return "Presiona el boton de RESOLVER para obtener los qubits resultado"
	if(phase == "QUANTIC_MEASURE"):
		return "Presiona el boton MEDIR para obtener el valor de los qubits en superposicion"
	return ""
...
func _on_button_pressed():
	...
	var card1
	if(FirstDraw==1):
		card1 = CardSceneArray[0].instantiate()
		FirstDraw = 2
	else:
		card1 = CardSceneArray[randi_range(0,CardSceneArray.size()-1)].instantiate()
...
func show_good_job_tutorial_then_hide() -> void:
	...
	TutorialTypewriterRunId = TutorialTypewriterRunId + 1
	$CanvasLayer/TutorialPanel.visible = true
	TutorialTextLabel.text = "¡Buen trabajo!"
